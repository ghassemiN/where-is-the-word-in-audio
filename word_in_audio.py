...
class WordInAudio():
	...
	def __init__(self, path, word_search, language):
		self.path = path
	...
	def word_estimated_time(self, chunks_name_list, chunks_duration_list, chunk_include_word):
	...
	def get_large_audio_transcription(self):
		...
		for i, audio_chunk in enumerate(chunks, start=1):
			...
			chunks_duration_list.insert(i, self.get_wav_duration(chunk_filename))
			# recognize the chunk
			with sr.AudioFile(chunk_filename) as source:
				...
				try:
					...
					if find_word != -1 :
						...
						begin_time , end_time = self.word_estimated_time(chunks_name_list, chunks_duration_list, chunk_include_word)
